...
DensLambda <- function(x){
  ...
  abline(v=mean(x$lambda) , col="slateblue")
...
DensPhi <- function(x){
  ...
  abline(v=mean(x$phi) , col="orange")
...
DensGamma <- function(x){
  ...
  dens( logistic(rnorm(50000, mean = 0, sd = 1)) , add=TRUE , lty=2 , col="black") #prior
  abline(v=mean(x$gamma) , col="green")
...
DensFc <- function(x){
  ...
  dens( exp(rnorm(50000, mean = 0, sd = 0.8)) , add=TRUE , lty=2 , col="black") #prior
  abline(v=mean(x$fc_i) , col="red")
...
DensBetaq <- function(x){
  ...
  abline(v=mean(x$betaq) , col="violet")
