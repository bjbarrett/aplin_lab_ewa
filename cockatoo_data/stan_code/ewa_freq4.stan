...
parameters {
  ...
  vector[N_effects] G[L];
  //G = (diag_pre_multiply(sigma_g,L_Rho_g) * zed_g)'; //cholesky decomp majick[N_effects,L] zed_g;                // individual z-scores for cholesky decomp
  corr_matrix[N_effects] Rho_g;  // correlation matrix
...
model {
   ...
   //priors
    ...
    G ~ multi_normal(  rep_vector(0,4) , quad_form_diag(Rho_g , sigma_g) );
    mu[1]~  normal(1,0.6);
    mu[2]~  normal(0,1);
    mu[3]~  normal(0,1);
    mu[4]~  normal(0,0.8);
    ...
    for ( i in 1:N ) {
        ...
        if ( bout[i]==1 ) {
            ...
            lambda = exp(mu[1] + I[id[i],1] + G[group_index[i],1] + log_lambda[age_index[i],sex_index[i]] );
            phi= inv_logit( mu[2] + I[id[i],2] + G[group_index[i],2] + logit_phi[age_index[i],sex_index[i]] );
            gamma = inv_logit(mu[3] + I[id[i],3] + G[group_index[i],3] + logit_gamma[age_index[i],sex_index[i]] );
            fc = exp(mu[4] + I[id[i],4] + G[group_index[i],4] + log_f[age_index[i],sex_index[i]]);
        }
...
generated quantities{
    ...
    for ( i in 1:N ) {
        ...
        if ( bout[i]==1 ) {
            lambda_i[id[i]] = exp( mu[1] + I[id[i],1] + G[group_index[i],1] + log_lambda[age_index[i],sex_index[i]] );
            phi_i[id[i]] = inv_logit( mu[2] +  I[id[i],2] + G[group_index[i],2] + logit_phi[age_index[i],sex_index[i]] );
            gamma_i[id[i]] = inv_logit(mu[3] + I[id[i],3] + G[group_index[i],3] + logit_gamma[age_index[i],sex_index[i]] );
            fc_i[id[i]] = exp(mu[4] + I[id[i],4] + G[group_index[i],4] + log_f[age_index[i],sex_index[i]]);
        }
