   ...
   //priors
    ...
    for ( i in 1:N ) {
        ...
        if ( bout[i]==1 ) {
            ...
            lambda = exp( I[id[i],1] + A[1,age_index[i]] ) ;
            phi= inv_logit( I[id[i],2] + A[2,age_index[i]] );
...
generated quantities{
    ...
    for ( i in 1:N ) {
        ...
        if ( bout[i]==1 ) {
            lambda_i[id[i]] = exp(  I[id[i],1] + A[1,age_index[i]]) ;
            phi_i[id[i]] = inv_logit(  I[id[i],2] + A[2,age_index[i]]);
        }
