data {
  ...
  real q[N,K];        // observed number of ttimes observing behaviors
...
parameters {
  vector<lower=0>[N_effects-1] sigma_i;       // standard deviations of varying effects
  matrix[N_effects-1,J] zed_i;                // individual z-scores for cholesky decomp
  cholesky_factor_corr[N_effects-1] L_Rho_i;  // correlation matrix
  // vector<lower=0>[N_effects-1] sigma_g;       // standard deviations of varying effects
  // matrix[N_effects-1,L] zed_g;                // individual z-scores for cholesky decomp
  // cholesky_factor_corr[N_effects-1] L_Rho_g;  // correlation matrix
...
transformed parameters{
  matrix[J,N_effects-1] I;              //define varying effects for individuals
  I = (diag_pre_multiply(sigma_i,L_Rho_i) * zed_i)'; //cholesky decomp majick
  matrix[L,N_effects-1] G;              //define varying effects for individuals
...
model {
  ...
  to_vector(zed_i) ~ normal(0,0.3);
  L_Rho_i ~ lkj_corr_cholesky(3);
  sigma_g[lower=0] ~ normal(0,1);
  to_vector(zed_g) ~ normal(0,0.3);
      ...
      // calculate new individual's parameter values
        ...
        //conformity aspect below
            if (sum( q[i] ) > 0 ) {
                ...
                for ( j in 1:K ) s_temp[j] = pow(q[i,j],1);
...
generated quantities{
    ...
    matrix[N_effects-1,N_effects-1] Rho_i;
    matrix[N_effects-1,N_effects-1] Rho_g;
    matrix[N,K] PrPreds;     
    ...
    for ( i in 1:N ) {
        ...
        //conformity aspect below
            if (sum( q[i] ) > 0 ) { //only socially learn when there is social information
                // compute frequency cue
                for ( j in 1:K ) s_temp[j] = pow(q[i,j],1);
                PrS = s_temp[tech[i]]/sum(s_temp);
