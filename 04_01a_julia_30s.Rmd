...
```{r il}
load("~/Dropbox/bias_i_sa_30/ind.rds")
fit_i <- stanfit
...
```{r fdsll}
load("~/Dropbox/bias_i_sa_30/freq.rds")
fit_freq <- stanfit
...
Lets calc % of posterior consistent with conformity.
```{r}
dem <- length(post_freq$log_f[,1,1])

length(which(post_freq$log_f[,1,1] > 0)) / dem #af
length(which(post_freq$log_f[,2,1] > 0)) / dem #jf
length(which(post_freq$log_f[,3,1] > 0)) / dem #uf

length(which(post_freq$log_f[,1,2] > 0)) / dem #am
length(which(post_freq$log_f[,2,2] > 0)) / dem #jm
length(which(post_freq$log_f[,3,2] > 0)) / dem #um

#unknown
length(which(post_freq$log_f[,1,3] > 0)) / dem #au
length(which(post_freq$log_f[,2,3] > 0)) / dem #ju
length(which(post_freq$log_f[,3,3] > 0)) / dem #uu
```
## Male Biased Linear Social Learning

```{r mblsl}
load("~/Dropbox/bias_i_sa_30/male_lin.rds")
...
```{r ablsl}
load("~/Dropbox/bias_i_sa_30/adult_lin.rds")
...
```{r roostb sl}
load("~/Dropbox/bias_i_sa_30/roost_cue.rds")
fit_roost<- stanfit
...
```{r  male b sl}
load("~/Dropbox/bias_i_sa_30/male_cue.rds")
fit_male <- stanfit
...
```{r  adult b sl}
load("~/Dropbox/bias_i_sa_30/adult_cue.rds")
fit_adult <- stanfit
